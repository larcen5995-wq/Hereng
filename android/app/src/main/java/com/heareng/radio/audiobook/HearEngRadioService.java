...
public class HearEngRadioService extends Service {
    ...
    public void setServiceSpeechRate(float rate) {
        if (rate >= 0.3f && rate <= 3.0f) {
            this.currentSpeechRate = rate;
            if (serviceTts != null && isServiceTtsReady) {
                try { serviceTts.setSpeechRate(rate); } catch(Exception e){}
            }
            if (isServicePlaying) {
                try {
                    JSONObject card = nativePlaylist.getJSONObject(currentCardIndex);
                    boolean isNovel = card != null && (card.optBoolean("isNovel", false) || "novel".equalsIgnoreCase(card.optString("type")));
                    if (isNovel) playNovelServiceLoop();
                    else playCurrentServiceStep();
                } catch(Exception e) {}
            }
        }
    }
                            ...
                            if (isServicePlaying) {
                                serviceHandler.post(new Runnable() {
                                    @Override public void run() {
                                        if (uid != null && uid.startsWith("HearEngNovelTTS_")) {
                                            advanceNovelCard();
                                        } else {
    ...
    public void playCurrentServiceStep() {
        ...
        try {
            ...
            JSONObject card = nativePlaylist.getJSONObject(currentCardIndex);
            boolean isNovelCard = card != null && (card.optBoolean("isNovel", false) || "novel".equalsIgnoreCase(card.optString("type")));

            // 📖 소설 소분류 전용 독립 오디오북 낭독 엔진으로 분기!
            if (isNovelCard) {
                playNovelServiceLoop();
                return;
            }
            ...
            if (serviceTts != null && isServiceTtsReady) {
                ...
                serviceTts.setSpeechRate(this.currentSpeechRate);
    ...
    // ═══════════════════════════════════════════════════════════════
    // 📖 소설 소분류 전용 독립 오디오북 낭독 엔진 (스킵 없는 스무스 연속 낭독)
    // ═══════════════════════════════════════════════════════════════
    public void playNovelServiceLoop() {
        requestPhysicalAudioFocus();
        if (!isServicePlaying || nativePlaylist == null || nativePlaylist.length() == 0) return;
        try {
            if (currentCardIndex >= nativePlaylist.length()) currentCardIndex = 0;
            ...
            JSONObject card = nativePlaylist.getJSONObject(currentCardIndex);
            String rawText = card.optString("target", "");
            if (rawText == null || rawText.trim().isEmpty()) rawText = card.optString("translation", "");
            String textToSpeak = cleanTextForTts(rawText);
            ...
            String notifyTitle = "📖 [소설 " + (currentCardIndex + 1) + " / " + nativePlaylist.length() + "]";
            String notifySub = textToSpeak;
            updateMediaMetadata(notifyTitle, notifySub,
                (long)(currentCardIndex + 1) * 1000L, (long)nativePlaylist.length() * 1000L);
            ...
            if (serviceTts != null && isServiceTtsReady) {
                boolean hasHangul = textToSpeak.matches(".*[\\uac00-\\ud7a3]+.*");
                boolean hasEnglish = textToSpeak.matches(".*[a-zA-Z]+.*");

                if (hasHangul || !hasEnglish) {
                    int res = serviceTts.setLanguage(Locale.KOREA);
                    if (res == TextToSpeech.LANG_MISSING_DATA || res == TextToSpeech.LANG_NOT_SUPPORTED) {
                        serviceTts.setLanguage(Locale.KOREAN);
                    }
                ...
                long speechEstMs = Math.max(3500L, textToSpeak.length() * 160L);
                if (ttsWatchdogRunnable != null) serviceHandler.removeCallbacks(ttsWatchdogRunnable);
                ttsWatchdogRunnable = new Runnable() {
                    @Override public void run() {
                        if (isServicePlaying) {
                ...
                int result = serviceTts.speak(textToSpeak, TextToSpeech.QUEUE_FLUSH, params,
                    "HearEngNovelTTS_" + System.currentTimeMillis());

                if (result != TextToSpeech.SUCCESS) {
                    if (ttsWatchdogRunnable != null) serviceHandler.removeCallbacks(ttsWatchdogRunnable);
                    serviceHandler.postDelayed(new Runnable() {
                        @Override public void run() { if (isServicePlaying) advanceNovelCard(); }
                    }, 400);
                }
            } else {
                initServiceTts();
                serviceHandler.postDelayed(new Runnable() {
                    @Override public void run() { if (isServicePlaying) playNovelServiceLoop(); }
                }, 300);
            }
        } catch (Exception e) { e.printStackTrace(); }
    }

    public void advanceNovelCard() {
        ...
        serviceHandler.postDelayed(new Runnable() {
            @Override public void run() {
                if (isServicePlaying) playNovelServiceLoop();
            }
        }, 250);
    }
