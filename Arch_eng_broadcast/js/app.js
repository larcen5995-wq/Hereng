...
function initVocalizeApp() {
  ...
  function initAppEdition() {
    ...
    // Auto-Restore Last Saved Bookmark Position on Startup!
  ...
  function updateCardUI(item, index) {
    ...
    let rawTranslation = item.translation || '';
    // If translation has format "englishWord (한글뜻)", extract Korean text from parentheses!
    const parenMatch = rawTranslation.match(/[\(\[\{]([\uac00-\ud7a3\s~,]+)[\)\]\}]/);
    if (parenMatch && parenMatch[1] && parenMatch[1].trim()) {
      rawTranslation = parenMatch[1].trim();
    } else {
      rawTranslation = rawTranslation.replace(/^[a-zA-Z\s\-]+(?=[\uac00-\ud7a3])/, '').trim();
    }
    // Clean noise symbols ; and :
    rawTranslation = rawTranslation.replace(/[:;]/g, ' ').replace(/\s+/g, ' ').trim();
    translationWord.textContent = rawTranslation || item.translation || '';
