...
class AdMobEngine {
  constructor() {
    ...
    this.prodBannerSlot = 'ca-app-pub-8036094597229084/8512243931';
    this.prodInterstitialSlot = 'ca-app-pub-8036094597229084/5225790958';
    ...
    // ★ [Free vs Pro 라이선스 정상화] ★
    // PRO 빌드이거나 window.isHearEngProEdition === true인 경우에만 PRO 적용!
    const isProBuild = document.body.classList.contains('is-pro-version') || window.isHearEngProEdition === true;

    this.isProUnlocked = false;

    if (this.isProUnlocked) {
      document.body.classList.add('is-pro-version');
    } else {
      document.body.classList.remove('is-pro-version');
      localStorage.removeItem('heareng_pro_unlocked');
    }
  ...
  init() {
    if (this.isProUnlocked) {
      console.log('👑 HearEng PRO Member detected - AdMob Ads 100% Disabled');
  ...
  renderLiveAdVisualizer() {
    const bannerContainer = document.getElementById('tab1SponsorAdBox');
  ...
  hideAllAdBanners() {
    const bottomAdBanner = document.getElementById('tab1SponsorAdBox');
