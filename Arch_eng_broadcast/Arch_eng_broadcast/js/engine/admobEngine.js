...
class AdMobEngine {
  constructor() {
    ...
    // Check if user has purchased PRO Lifetime License or running PRO App (.pro)
    const isProApp = (window.Capacitor && window.Capacitor.config && window.Capacitor.config.appId && window.Capacitor.config.appId.endsWith('.pro')) || 
                     (window.location && window.location.href.includes('.pro')) ||
                     document.body.classList.contains('is-pro-version');

    this.isProUnlocked = localStorage.getItem('heareng_pro_unlocked') === 'true' || isProApp;
  ...
  startRadioCmTimer() {
    ...
    this.radioCmInterval = setInterval(() => {
      ...
      // 1. Daily 7 AM Ad Check (아침 7시 이후 1회)
      ...
      if (currentHour >= 7 && currentHour < 19 && last7Am !== todayStr) {
        ...
        this.triggerDailyAdMent("광고 시간입니다. 광고 없는 버전도 판매 중입니다.");
        return;
      }

      // 2. Daily 7 PM Ad Check (저녁 7시 이후 1회)
      const last7Pm = localStorage.getItem('last_7pm_ad_date');
      if (currentHour >= 19 && last7Pm !== todayStr) {
  ...
  closeWebAdModalAndResume() {
    ...
    // Auto-resume reading current card
    if (window.ttsEngine) {
      window.ttsEngine.isPlaying = true;
      setTimeout(() => {
        window.ttsEngine.speakCurrentCardWebBrowser();
      }, 300);
    }
  }
